...
//int xbstat = msr.getStatus(); 
 ...
 nunchuk.update();                // ALL data from nunchuk is continually sent to Receiver
          joyx = map(nunchuk.analogX, chan1Min, chan1Max, 0, 179); // Channel 1 joyx & Channel 2 joyy from NunChuck Joystick
          joyy = map(nunchuk.analogY, chan2Min, chan2Max, 0, 179); // Map it to Min & Max of each channel
...
//  tft.drawHorizontalLine(0, 19, tft.width(), BLUE);  // we should be able remove this from here
...
// removed the code for now, since the XBee Series 2 in API mode is handling this quite nicely...

// after sending a tx request, we expect a status response wait up to half second for the status response

/* //  -------------   XBee Series 2 API Response Handler BEGIN
  if (xbee.readPacket(500)) {                                        // got a response!
//  Serial.print("Received XBee Response");           	             // DEBUG CODE
    if (xbee.getResponse().getApiId() == ZB_TX_STATUS_RESPONSE) {    // should be a znet tx status
      xbee.getResponse().getZBTxStatusResponse(txStatus);
      if (txStatus.getDeliveryStatus() == SUCCESS) {                 // get the delivery status, the fifth byte
//      Serial.print("ZB TX Success");           	             // DEBUG CODE       
      } else {
//      Serial.print("ZB TX Failed");           	             // DEBUG CODE       
...
*/  //  -------------   XBee Series 2 API Response Handler END  
}  
