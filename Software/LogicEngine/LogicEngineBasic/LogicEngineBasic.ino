// BASIC SKETCH FOR THE RSERIES LOGIC ENGINE
// This sketch provides basic logic functions (the random logic pattern) for front or rear logics.
// It does not utilize the microSD card or have any communication/I2C functions.
...
byte numLeds,totalColors,Keys; //numLeds will be the ACTUAL number of LEDs (80 for FLD, 96 for RLD)
...
const byte rldColors[5][3] = { {87,0,0} , {87,206,105} , {79,255,184} , {38,255,250} , {255,255,214} };
...
31,30,29,28,27,26,25,24,79,78,77,76,75,74,73,72,
32,33,34,35,36,37,38,39,80,81,82,83,84,85,86,87};
// for 48-LED boards (only lower 40 used for FLDs)...
//byte fldMap[80]={15,14,13,12,11,10,9,8,16,17,18,19,20,21,22,23,31,30,29,28,27,26,25,24,32,33,34,35,36,37,38,39,47,46,45,44,43,42,41,40};
...
void setup() {
      ...
      if (digitalRead(TOGGLEPIN)==HIGH) {
        ...
        #endif
        logic=1;
        numLeds=80; //96 for final version
      ...
      if (digitalRead(SPEEDPIN)==HIGH) speeds=1;  
      
      //make a giant array of all colors tweened...
      byte el,val,kcol,perStep,LEDnum,totalColorCount; //
      ...
      for(byte x=0;x<numLeds;x++) {
        ...
        //now set this LED up...
        leds[x].setHSV(AllColors[LEDstat[x][0]][0],AllColors[LEDstat[x][0]][1],AllColors[LEDstat[x][0]][2]);   
        FastLED.show(); 
        delay(10);
        if (x>0) leds[x-1].setHSV(0,0,0);
        if (x==numLeds-1) leds[x].setHSV(0,0,0);
      }
...
void loop() {
    ...
    for(byte LEDnum=0;LEDnum<numLeds;LEDnum++) updateLED(LEDnum);  
    FastLED.show();
    //delay(20);
}
