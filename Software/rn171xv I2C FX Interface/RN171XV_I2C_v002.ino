...
*/
char* xvCodes[10] = {"*OPEN","*CLOS",":CL00",":OP00",":SE01"":SE15","*ON00"};
int i2cAddress[] = {0, 0, 12, 12, 3, 3, 1};
int i2cEvent[] =   {0, 0,  0,  1, 3, 1, 1};
...
void sendEvent() {
  if (xvpacketvalid == true) {
    ...
    Serial.print(i2cAddress[rxCode]);
    Serial.print(" Event: \t");
    Serial.println(i2cEvent[rxCode]);
    
//    Wire.beginTransmission(i2cAddress[rxCode]);  // transmit to device  which is the Audio FX 2 Module rMP3
//    Wire.write(i2cAddress[rxCode]); // sends Trigger Event LSB 
